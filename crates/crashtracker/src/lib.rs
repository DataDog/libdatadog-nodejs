...
#[napi]
pub fn report_unhandled_exception(_env: Env, error: JsObject) -> napi::Result<()> {
    let exception_type = get_optional_string_property(&error, "name")?;
    let exception_message = get_optional_string_property(&error, "message")?;
    let stack_string = get_optional_string_property(&error, "stack")?;

    let stacktrace = match &stack_string {
        Some(s) => parse_v8_stack(s),
        None => libdd_crashtracker::StackTrace::empty(),
    };
